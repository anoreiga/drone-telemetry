...
public class ContainerMenuItem extends RadialMenuItem {
    ...
    public ContainerMenuItem(final double menuSize, final Node graphic) {
	super(menuSize, graphic);
	this.initialize();
    }

    public ContainerMenuItem(final double menuSize, final String text,
	    final Node graphic) {
	...
	this.visibleProperty().addListener(new ChangeListener<Boolean>() {
		...
		if (!arg0.getValue()) {
		    ContainerMenuItem.this.childAnimGroup
			    .setVisible(false);
		    ContainerMenuItem.this.setSelected(false);
		}
	    }
	});
